...
// 获取浏览历史
$history_sql = "SELECT articles.*, users.username, users.avatar, browsing_history.viewed_at 
               FROM browsing_history 
               JOIN articles ON browsing_history.article_id = articles.id 
               JOIN users ON articles.author_id = users.id 
               WHERE browsing_history.user_id = '$user_id' 
               ORDER BY browsing_history.viewed_at DESC";
$history_result = mysqli_query($link, $history_sql);
                ...
                <table class="history-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo nl2br(htmlspecialchars(substr($item['content'], 0, 100))); ?>...</td>
                            <td>
                                <?php echo htmlspecialchars($item['username']); ?>
                                <?php if ($item['avatar']): ?>
                                    <img src="<?php echo htmlspecialchars($item['avatar']); ?>" class="author-avatar">
                                <?php endif; ?>
                            </td>
                            <td><?php echo $item['viewed_at']; ?></td>
                            <td>
                                <a href="article_view.php?id=<?php echo $item['id']; ?>" class="action-link">查看</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
